...
def dynamic_price_predictor(input_data: dict, model, feature_columns, demand_threshold=0.8, surge_rate=0.1, fare_cap_multiplier=1.5):
    import pandas as pd
    import numpy as np
    ...
    if base_fare > 0:
        final_fare = min(max(predicted_fare, base_fare), base_fare * fare_cap_multiplier)
    else:
...
def dynamic_price_predictor(input_data, model, feature_columns, demand_threshold=0.8, surge_rate=0.1, fare_cap_multiplier=1.5):
    input_df = pd.DataFrame([input_data])
    ...
    # Keep fare within the base fare and capped surge range
    base_fare = input_data.get("base_fare", 0)
    if base_fare > 0:
        final_fare = min(max(fare, base_fare), base_fare * fare_cap_multiplier)
    else:
        final_fare = fare
...
def dynamic_price_predictor_ensemble(input_data, rf_model, xgb_model, feature_columns, demand_threshold=0.8, surge_rate=0.1, fare_cap_multiplier=1.5):
    import numpy as np
    import pandas as pd
